...
{
//    [self performSelector:@selector(cusCature:) withObject:viewController afterDelay:0.1];
...
{
    SBBaseViewController *vc = (SBBaseViewController *)[self topViewController];
    
    [self.screenShotsList removeLastObject];
    
    if ([vc isKindOfClass:[SBBaseViewController class]]) [vc clearViewController];
...
- (void)cusCature:(UIViewController *)viewController
{
    UIImage *capturedImage = [self capture];
...
- (UIImage *)capture
{
    ...
    if ([IT_SYSTEM_VERSION floatValue] >= 7.0) {
        [TOP_VIEW drawViewHierarchyInRect:self.view.bounds afterScreenUpdates:YES];
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if ([vc isKindOfClass:[SBBaseViewController class]]) {
        SBBaseViewController *sbvc = (SBBaseViewController *)vc;
        return sbvc.isPopable;
    }
