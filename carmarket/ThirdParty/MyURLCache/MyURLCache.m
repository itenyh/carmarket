...
+ (void)initialize
{
    ...
    NSString *dicPath = [cacheDirectory stringByAppendingString:@"/MyURLCache"];
    if (![fileManager fileExistsAtPath:dicPath]) {
        [fileManager createDirectoryAtPath:dicPath withIntermediateDirectories:NO attributes:nil error:nil];
    }
...
//        NSString *path = [cacheDirectory stringByAppendingString:@"Info.plist"];
        NSString *path = [self cacheDirectoryPath:@"Info.plist"];
...
//        NSString *path = [cacheDirectory stringByAppendingString:[responseInfo objectForKey:@"filename"]];
        NSString *path = [self cacheDirectoryPath:[responseInfo objectForKey:@"filename"]];
        NSFileManager *fileManager = [NSFileManager defaultManager];
        if ([fileManager fileExistsAtPath:path])
        {
            ...
            NSURLResponse *response = [[NSURLResponse alloc] initWithURL:request.URL MIMEType:[responseInfo objectForKey:@"MIMEType"] expectedContentLength:data.length textEncodingName:@"utf-8"];
   ...
   // NSData *data = [NSURLConnection sendSynchronousRequest:newRequest returningResponse:&response error:&error];
    [NSURLConnection sendAsynchronousRequest:newRequest queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data,NSError *error){
        ...
        NSString *path = [self cacheDirectoryPath:filename];
...
- (NSString *)cacheDirectoryPath:(NSString *)name
{
    return [cacheDirectory stringByAppendingString:[NSString stringWithFormat:@"/MyURLCache/Caches%@", name]];
}
